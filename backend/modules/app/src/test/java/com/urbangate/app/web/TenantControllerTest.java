...
class TenantControllerTest {
    ...
    @MockitoBean
    TenantDiscoveryKeyCloakService tenantService;
    ...
    class ProvisionTenant {
        ...
        void provision_validTenant_returns201() throws Exception {
            willDoNothing()
                    .given(tenantService).provisionTenant(eq("acme"), any(TenantConfig.class));

            mockMvc.perform(post(BASE_URL + "/acme")
    ...
    class DeleteTenant {
        ...
        void delete_existingTenant_returns200() throws Exception {
            willDoNothing()
                    .given(tenantService).deleteTenant("acme");

            mockMvc.perform(delete(BASE_URL + "/acme")
                            .header("X-Tenant-ID", TENANT_HEADER))
                    .andDo(print())
        ...
        void delete_nonExistentTenant_returns404() throws Exception {
            willThrow(new ResourceNotFoundException("Tenant 'ghost' not found"))
                    .given(tenantService).deleteTenant("ghost");
